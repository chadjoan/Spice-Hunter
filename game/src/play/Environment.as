...
public class Environment
  ...
  private var screenDrawProfiler:Profiler = new Profiler("Drawing Time");
  private var logicTimeProfiler:Profiler  = new Profiler("Logic Time");
  private var tetherTimeProfiler:Profiler = new Profiler("Tether Time");
  ...
  public function Environment (_shipspecs:Array, round : Number )
    ...
    vmTimeProfiler.y     = 30;
    gameTimeProfiler.y   = 60;
    screenDrawProfiler.y = 90;
    logicTimeProfiler.y  = 120;
    tetherTimeProfiler.y = 150;

    // hack hack
    Screen.swfs.addChild( totalTimeProfiler );
    Screen.swfs.addChild( vmTimeProfiler );
    Screen.swfs.addChild( gameTimeProfiler );
    Screen.swfs.addChild( screenDrawProfiler );
    Screen.swfs.addChild( logicTimeProfiler );
    Screen.swfs.addChild( tetherTimeProfiler );
  ...
  public function onEnterFrame() : void
    ...
    vmTimeProfiler.endBench();
    gameTimeProfiler.startBench();

    totalTimeProfiler.endBench();
    totalTimeProfiler.startBench();

    screenDrawProfiler.startBench();
    Screen.draw();
    ...
    for (loop1=0; loop1 < spices.length; loop1++)
      for (loop2=0; loop2 < homebases.length; loop2++)
        ...
        if ( collisionCheck(spices[loop1], homebases[loop2]) )
          ...
          Utility.playSound( Assets.Score );
          ...
          if (spices[loop1].fingerprint != null)
            ...
            if (spices[loop1].fingerprint.releaser.spec.teamCode == homebases[loop2].spec.teamCode)
              ...
              if (dl_bonus > 200.0)
                ...
                Utility.playSound (Assets.nice_shot );
                var ns : NiceShot = new NiceShot ( spices[loop1].fingerprint.releaser.spec.teamCode,
          ...
          if (spices[loop1].mass > 10.0)
            ...
            var ml : Motherlode = new Motherlode (homebases[loop2].spec.teamCode);
            ml.radius = 50;
            ...
            Utility.playSound (Assets.bigspicebonus );
            }
    ...
    for (loop1=0; loop1 < asteroids.length; loop1++)
      for (loop2=loop1+1; loop2 < asteroids.length; loop2++)
        ...
        if ( collisionCheck (asteroids[loop1], asteroids[loop2]) )
          ...
          Utility.playSound( Assets.AstAstCollide );
      ...
      for (loop2=0; loop2 < spices.length; loop2++)
       if (!asteroids[loop1].isExpired && !spices[loop2].isExpired)
        if ( collisionCheck (asteroids[loop1], spices[loop2]) )
          {
          Utility.playSound( Assets.AstSpiceCollide );
    ...
    for (loop1=0; loop1 < spices.length; loop1++)
      for (loop2=loop1+1; loop2 < spices.length; loop2++)
        ...
        if ( collisionCheck (spices[loop1], spices[loop2]) )
          ...
          var sg : SpiceGlob = new SpiceGlob();
          ...
          if (spices[loop1].fingerprint != null)
            ...
            var spicecombo : SpiceComboAnim = new SpiceComboAnim(spices[loop1].fingerprint.releaser.spec.teamCode);
            spicecombo.radius = 50;
            spicecombo.boundTo = spices[loop1];
            animations.push(spicecombo);
            // Play the spice combo sound.
            Utility.playSound (Assets.combosound);
            }
          ...
          if (listOfTetherers.length > 0)
            ...
            if (allAreAllied)
              ...
              var spicecombo2 : SpiceComboAnim = new SpiceComboAnim(listOfTetherers[0].spec.teamCode);
          ...
          Utility.playSound( Assets.SpiceSpiceCollide );
          }
  ...
  public function removeExpiredThings () : void
    ...
    while (loop1 < asteroids.length)
      if (asteroids[loop1].isExpired)
        ...
        for (loop2 = 0; loop2 < ships.length; loop2++)
          {
          if (ships[loop2].attractBeam.target == Body(asteroids[loop1]) )
            {
            ships[loop2].attractBeam.target = null;
            ships[loop2].triggerReset(Ship.GRAVITY_SNAP_EVENT);
            }
          if (ships[loop2].repelBeam.target == Body(asteroids[loop1]) )
            {
            ships[loop2].repelBeam.target = null;
    ...
    while (loop1 < spices.length)
      if (spices[loop1].isExpired)
        ...
        for (loop2 = 0; loop2 < ships.length; loop2++)
          {
          if (ships[loop2].attractBeam.target == Body(spices[loop1]) )
            {
            ships[loop2].attractBeam.target = null;
            ships[loop2].triggerReset(Ship.GRAVITY_SNAP_EVENT);
            }
          if (ships[loop2].repelBeam.target == Body(spices[loop1]) )
            {
            ships[loop2].repelBeam.target = null;
            ships[loop2].triggerReset(Ship.REPEL_SNAP_EVENT);
            }
  ...
  public function reticleSearch() : void
    ...
    for (var r:Number = 0; r < 4; r++)
      if ( ships[r].reticleLockingOn )
        ships[r].reticle.setSticky (findBody(ships[r].reticle.rx, ships[r].reticle.ry, ships[r].reticle.radius*0.5 ));
    }
