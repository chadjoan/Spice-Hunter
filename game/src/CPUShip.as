...
public class CPUShip extends Ship
  ...
  public function do_NAVIGATION_DECIDING() : void
    ...
    if (gravityTarget != null)
      {
      gravity_x = (gravityTarget.rx - rx);
      gravity_y = (gravityTarget.ry - ry);
      var G:Number = Math.sqrt(gravity_x*gravity_x + gravity_y*gravity_y);        
      gravity_x = gravity_x / G * gravityStrength;
      gravity_y = gravity_y / G * gravityStrength;
      }
    ...
    if (repelTarget != null) 
      {
      repel_x = -(repelTarget.rx - rx);
      repel_y = -(repelTarget.ry - ry);
      var R:Number = Math.sqrt(repel_x*repel_x + repel_y*repel_y);        
      repel_x = repel_x / R * repelStrength;
      repel_y = repel_y / R * repelStrength;           
      }
    ...
    for (i = 0; i < bodyBuffer.length; i++)
      {
      resultant_x = normals_x[i]*gravityStrength + repel_x;
      resultant_y = normals_y[i]*gravityStrength + repel_y;	        
    ...
    for (i = 0; i < bodyBuffer.length; i++)
      {                
      resultant_x = gravity_x - normals_x[i]*repelStrength;
      resultant_y = gravity_y - normals_y[i]*repelStrength;	        
      U = Math.sqrt(resultant_x*resultant_x + resultant_y*resultant_y);
      if (U > 1e-3)
        {
        resultant_x /= U;
    ...
    for (i = 0; i < bodyBuffer.length; i++)
      {
      resultant_x = normals_x[i]*gravityStrength;
      resultant_y = normals_y[i]*gravityStrength;	        
      U = Math.sqrt(resultant_x*resultant_x + resultant_y*resultant_y);
      if (U > 1e-3)
        {
        resultant_x /= U;
    ...
    for (i = 0; i < bodyBuffer.length; i++)
      {                
      resultant_x = -normals_x[i]*repelStrength;
      resultant_y = -normals_y[i]*repelStrength;	        
      U = Math.sqrt(resultant_x*resultant_x + resultant_y*resultant_y);
      if (U > 1e-3)
        {
        resultant_x /= U;
    ...
    if (reticleAI.isReady)
      ...
      switch (moveCode)
        ...
        case HOLD_REPEL_MOVE_GRAVITY:
          gravityTarget = reticleAI.target;
        break;
  ...
  public function do_NAVIGATION_PROJECT_DECIDE () : void
    ...
    ai_state = NAVIGATION_DECIDING;      
    if (errorDot > 0 && Math.abs(errorDot) > gravityStrength)        
      ai_state = NAVIGATION_PROJECT_PULL;
    if (errorDot < 0 && Math.abs(errorDot) > repelStrength)        
  ...
  public function computeVelocityAndNeighborhood() : void
    ...
    if (desiredTriangle == myTriangle)
      ...
      if (!collisionPossible)
        ...
        best = env.map.shortestPath (this, desired_Body,Screen.midground);                             
