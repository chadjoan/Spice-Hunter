package
{
	...
	public class GarageMenu extends Sprite
	{
		...
		public function GarageMenu(xOffset:Number, yOffset:Number, _specs:ShipSpec)
		{
			specs = _specs;
			x = xOffset;
			y = yOffset;
		...
		public function navigateUp(event:ControlEvent) : void
		{
		  if(!debouncer.debounced())
			...
			if(!isDone)
			{
				...
				Utility.playSound(new Assets.menuScroll());
			}
		}
		
		public function navigateDown(event:ControlEvent) : void
		{
		  if(!debouncer.debounced())
		    return;
		  else
		...
		public function upgrade(event:ControlEvent) : void
		{
		  if(!debouncer.debounced())
		    return;
		  else
			...
			else
			{
				...
				if(items[selection].level != 10)
				{
					...
					Utility.playSound (new Assets.menuRight() );
		...
		public function downgrade(event:ControlEvent) : void
		{
		  if(!debouncer.debounced())
		    return;
		  else
			...
			else
			{
				...
				if(items[selection].level != 1)
				{
					...
					Utility.playSound (new Assets.menuLeft() );
					items[selection].update(items[selection].level - 1);
					switch (selection)
					  {
					  case 0: specs.gravityLevel -= 1; break;	
