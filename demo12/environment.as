class Environment
  ...
  public function Environment ()
    ...
    for (var a:Number=1; a < 7; a++)
      ...
      // Randomize position.
      asteroids[asteroids.length-1].rx = 100*a;
      asteroids[asteroids.length-1].ry = 50;
      // Randomize velocity.     
      asteroids[asteroids.length-1].vx = 0;
      asteroids[asteroids.length-1].vy = 0;
      
      asteroids.push(new Asteroid());
      // Randomize position.
      asteroids[asteroids.length-1].rx = 100*a;
      asteroids[asteroids.length-1].ry = 550;
      // Randomize velocity.     
      asteroids[asteroids.length-1].vx = 0;
      asteroids[asteroids.length-1].vy = 0;
    ...
    */
    ...
    // Make a structured initial field of stationary asteroids.
    /*
    for (var ix:Number=0; ix <3; ix++)
      for (var iy:Number=0; iy < 3; iy++)
        {
        asteroids.push (new Asteroid());	
        asteroids[asteroids.length-1].rx = ix*250+150;
        asteroids[asteroids.length-1].ry = iy*150+150;
        asteroids[asteroids.length-1].vx = 0;
        asteroids[asteroids.length-1].vy = 0;        
        }    
    asteroids[1].rx -= 50;
 ...
 */
    ...
    // Make a random initial field of Spices.
    ...
    ships[1].gravityTarget = homebases[1];
    ships[1].repelTarget = homebases[1];
  ...
  public function onEnterFrame() : Void
    ...
    // Update AI for all CPU controlled ships. 
    ships[1].updateAI();                 
        
    // Update ship velocities.
    for (var sh:Number=0; sh < ships.length; sh++)
      ships[sh].updateVelocity();
    ...
    for (var a1:Number=0; a1 < asteroids.length; a1++)
      for (var a2:Number=a1+1; a2 < asteroids.length; a2++)
        if ( collisionCheck (asteroids[a1], asteroids[a2]) )
          ...
          momentumExchangeAngular (asteroids[a1], asteroids[a2]);
          }
    ...
    for (var a:Number=0; a < asteroids.length; a++)
      for (var s:Number=0; s < spices.length; s++)
        if ( collisionCheck (asteroids[a], spices[s]) )
          ...
          momentumExchangeAngular (asteroids[a], spices[s]);
          }
  ...
  public function initializeMap ()
    ...
    // Give ships references to the new map.    
    ships[1].map = map;  
    ships[1].env = this;  
    }
  ...
  public function findBodies (center:Body, radius:Number) : Array
    // Find all objects with this circle.
    {
    var list:Array = new Array;
    // Add asteroids.
    for (var a:Number=0; a < asteroids.length; a++)    
      {
      var L:Number = (asteroids[a].rx - center.rx)*(asteroids[a].rx - center.rx) + (asteroids[a].ry - center.ry)*(asteroids[a].ry - center.ry);
      L = Math.sqrt(L);
      if (L < radius)
    ...
    for (var hb:Number=0; hb < homebases.length; hb++)    
      {
      var L:Number = (homebases[hb].rx - center.rx)*(homebases[hb].rx - center.rx) + (homebases[hb].ry - center.ry)*(homebases[hb].ry - center.ry);
      L = Math.sqrt(L);
      if (L < radius)
